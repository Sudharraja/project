...
if(isset($_REQUEST['save']))
{
  ...
  //end of of varibale and value setting
	...
	$query = "INSERT INTO `basic_details`( `name`, `user_code`, `address`, `email`, `mobile` , `profile` , `objective` ,`declaration`) VALUE ('$name','$user_code ','$address','$email','$mobile', '$profile', '$objective', '$declaration')";  
...
if(isset($_REQUEST['edit'])){
  ...
  //end of of varibale and value setting
  //start of typing  query
	$query = "UPDATE `basic_details` SET `name`='$name',`user_code`='$usernaeme',`address`='$address',`email`='$email',`mobile`='$mobile',`profile`='$profile',`objective`='$objective',`declaration`='$declaration' WHERE id='$id'";  
  // end of typing  query
	$message=iud_data($query);
  if($message=="sucess"){
		echo"<script>alert('data saved successfully ')</script>";
        ...
        $("body").on("click", "#btnExport", function () {
            html2canvas($('#example1')[0], {
                onrendered: function (canvas) {
                    ...
                    pdfMake.createPdf(docDefinition).download("basic_details.pdf");
                }
            });
        });
    </script>
